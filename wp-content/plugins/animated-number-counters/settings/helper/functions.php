...
function anc_6310_text_domain() {
   load_plugin_textdomain( 'animated-number-counters', false, ANC_6310_PLUGIN_LANGUAGE_PATH );
}
...
function anc_6310_special_character_remove($data) {
   ...
   function anc_6310_link_css_js()
   {
      ...
      wp_enqueue_script('anc-6310-codemirror-js', 'https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.48.0/codemirror.min.js', array('jquery'));
   }
   ...
   function anc_6310_number_counter_install()
   {
      ...
      $install_default = anc_6310_get_option( 'anc_6310_install_default');
      if(!$install_default){
         $wpdb->query("INSERT INTO {$wpdb->prefix}options(option_name, option_value) VALUES ('anc_6310_install_default', 1)");
      } else{
         return;
      }

      $counter = $wpdb->query("select id from $counter_table LIMIT 1");
      if(!$counter && !$install_default){
         ...
         ('Work Completed', '{$description}', '10000', '500', 'fas fa-globe', 'Read More', 'https://www.wpmart.com/accordion-faq-pro', 'https://wpmart.org/wp-content/uploads/anc/work-completed.png')
         ";
         $wpdb->query($sql);

         $sql = "INSERT INTO {$counter_table} (title, description, numbers, icons, readmore_text, readmore_url, image) VALUES
         ('Lines Of Code', '{$description}', '1427', 'fas fa-book-open', 'Read More', 'https://www.wpmart.com/accordion-faq-pro', 'https://wpmart.org/wp-content/uploads/anc/work-completed.png')
         ";
         $wpdb->query($sql);

         $sql = "INSERT INTO {$counter_table} (title, description, numbers, icons, readmore_text, readmore_url, image) VALUES
         ('Cups Of Coffee', '{$description}', '2100', 'fas fa-university', 'Read More', 'https://www.wpmart.com/accordion-faq-pro', 'https://wpmart.org/wp-content/uploads/anc/work-completed.png')
         ";
         $wpdb->query($sql);
      }
   }
...
?>
      ...
      <div class="anc_6310_add_media">
         ...
         <div class="anc_6310_add_media_body_shortcode">
            <input type="text" onclick="this.setSelectionRange(0, this.value.length)" value='[anc_6310_number_counter id="<?php echo esc_attr($id); ?>"]' />
         </div>
      </div>
      <br />
      <div class="anc_6310_add_media">
       ...
       <div id="anc_6310_add_or_edit_counter_modal" class="anc-6310-modal" style="display: none">
         <div class="anc-6310-modal-content anc-6310-modal-md">
            ...
            <?php wp_nonce_field("anc_6310_nonce_add_edit_counter") ?>
               <input type="hidden" name="add_edit_counter_id" value="<?php echo esc_attr($id) ?>" />
                  ...
                  <table border="0" width="100%" cellpadding="0" cellspacing="0" class="anc-member-table">
                     ...
                     if ($counterArray) {                                 
                        foreach ($counterArray as $allCounter) {
                           ...
                           echo "<tr class='anc-row-select' id='".esc_attr($allCounter['id'])."' height='40'>";
                           echo "<td width='20'><input type='checkbox' name='counterid[]' value='".esc_attr($allCounter['id'])."' id='chk-box-".esc_attr($allCounter['id'])."' class='anc-row-select-checkbox' {$checked} /></td>";
                           echo "<td>".esc_html(anc_6310_special_character_remove($allCounter['title']))."</td>";
                           echo "<td align='center' width='100'>".esc_html($allCounter['numbers'])."</td>";
                           echo '<td width="60" align="center">' . (($allCounter['icons']) ? "<i class='".esc_attr($allCounter['icons'])."' style='font-size: 20px'></i>" : '---') . '</td>';
                           echo "</tr>";                           
                        }
                     }
                     else {
                        echo "
                              <tr>
                                 <td colspan='3'>
                                       <h4><i class='fas fa-exclamation-circle'></i> No counter profile available. <a href='" . esc_url(admin_url('admin.php?page=anc-6310-accordion-add-edit')) . "' class='anc-6310-btn-success' id='add-accordion'>Create Counter Profile</a></h4>
                                 </td>
                              </tr>
                           ";
                     }                   
      ...
      <div id="anc_6310_rearrange_counter_modal" class="anc-6310-modal" style="display: none">
         <div class="anc-6310-modal-content anc-6310-modal-sm">
            ...
            <?php wp_nonce_field("anc_6310_nonce_rearrange_counter") ?>
               <input type="hidden" name="rearrange_counter_id" value="<?php echo esc_attr($id) ?>" />
               <input type="hidden" name="rearrange_counter_list" id="rearrange_counter_list" value="<?php echo esc_attr(implode(',', $counter_order)) ?>" />
               <div class="anc-6310-modal-header">
                  Rearrange Counter
                  <span class="anc-6310-close">&times;</span>
               </div>
                     ...
                     <?php   
                        if($counterOrderList) {
                           foreach ($counterOrderList as $allCounter) {
                              echo "<li class='ui-state-default anc-6310-ui-state-default' id='". esc_attr($allCounter['id']) ."'>". esc_html(anc_6310_special_character_remove($allCounter['title'])) ."</li>";
                           }        
                        }
                        else {
                           echo "
                              <div>
                                 <h4 style='text-align: center'><i class='fas fa-exclamation-circle'></i> No counter profile available. <a href='" . esc_url(admin_url('admin.php?page=anc-6310-accordion-add-edit')) . "' class='anc-6310-btn-success' id='add-accordion'>Create Counter Profile</a></h4>
                              </div>
                           ";
                        }      
                     ?>
   ...
   function anc_6310_no_preview_available() {
      echo '<div class="anc-6310-no-preview-available">No preview available <br />for this option.</div>';
   }
   ...
   function anc_6310_fa_icon_list($startTag, $endTag) {
      ...
      foreach ($iconArray as $iKey => $iValue) {
         ...
         $name = esc_html(ucwords($name[0]));
         $icons .= "<{$startTag} data-icon-name='". esc_attr($iValue[0]) ."'> {$name}<i class='". esc_attr($iKey) ."'></i>{$endTag}";
      }
      return $icons;
   }   
   ...
   function anc_6310_icon_image_load($allCounter) {
      if($allCounter['icon_type']) {
         echo "<img src='" . esc_url($allCounter['custom_icon']) . "' />";
      } else{
         echo "<i class='" . esc_attr($allCounter['icons']) . "'></i>";
      }
   }   
   ...
   function anc_6310_background_setting($bgType, $cssData) {
            ...
            <select name="background_type" id="background_type" class="anc-6310-form-input">
               <?php
                  for($i = 1; $i < count($bgTypeList); $i++) {
                     ...
                     echo "<option value='".esc_attr($i)."'{$selected}>".esc_html($bgTypeList[$i])."</option>";
                  }
               ?>
            </select>
         </td>
      ...
      <tr height="45" id='background-type-2'>
         ...
         <td>
            <input type="text" name="template_background_color" class="anc-6310-form-input anc_6310_color_picker" step="1" data-opacity="0" data-format="rgb" value="<?php echo isset($cssData['template_background_color']) ? esc_attr($cssData['template_background_color']) : 'rgba(255, 255, 255, 0)' ?>">
         </td>
      </tr>
      <tr height="45" id='background-type-3'>
         ...
         <td>
         <input type="text" name="box_background_image" id="anc_6310_box_background_image" class="anc-6310-form-input" data-opacity=".8" data-format="rgb" value="<?php echo isset($cssData['box_background_image']) ? esc_url($cssData['box_background_image']) : '' ?>">
      ...
      <tr height="45" id='background-type-4'>
         ...
         <td>
         <input type="text" name="youtube_video_url" class="anc-6310-form-input" value="<?php echo isset($cssData['youtube_video_url']) ? esc_url($cssData['youtube_video_url']) : '' ?>">
         </td>
      </tr>
      <?php
   } 
   ...
   function anc_6310_counter_suffix($data, $ids){
      if(!$data) return;
      echo "<div class='anc-6310-counter-" . esc_attr($ids) . "-count-suffix'>" . esc_attr(anc_6310_special_character_remove($data)) . "</div>";
   }   

   function anc_6310_counter_description($data, $ids){
      if(!$data) return;
      echo "<div class='anc-6310-counter-" . esc_attr($ids) . "-count-subtitle'>".esc_html(anc_6310_special_character_remove($data))."</div>";
   }   
   function anc_6310_counter_button_section($url, $data, $ids){
      if(!$data) return;
      echo "<div class='anc-6310-counter-" . esc_attr($ids) . "-button'><a href='" . esc_url($url) . "' target='_blank'>" . esc_attr(anc_6310_special_character_remove($data)) . "</a></div>";
   }   

   function anc_6310_get_option($name) {
      global $wpdb;
   ...
   if (!function_exists('anc_6310_version_status')) {
      function anc_6310_version_status() {
         global $wpdb;
         $db_version = anc_6310_get_option( 'anc_6310_version_info');
         if(!$db_version){
            ...
            $key = anc_6310_get_option( 'anc_6310_license_key');
   ...
   if (!function_exists('anc_6310_check_license')) {
      function anc_6310_check_license($key, $autoUpdate = false)
      {
         ...
         $db_key = anc_6310_get_option('anc_6310_license_key');
         if(!$db_key){
   ...
   function anc_6310_multi_language_set_all_data(){
      global $wpdb;
      $counter_table = $wpdb->prefix . 'anc_6310_counter';
      $data = $wpdb->get_results('SELECT * FROM ' . $counter_table . ' ORDER BY id DESC', ARRAY_A);
      foreach ($data as $value) {
         esc_html__(anc_6310_multi_language_get('Title', $value['title'], $value['id']), 'animated-number-counters');
         esc_html__(anc_6310_multi_language_get('Description', $value['description'], $value['id']), 'animated-number-counters');
         esc_html__(anc_6310_multi_language_get('Read-More', $value['readmore_text'], $value['id']), 'animated-number-counters');
         $commons = $value['commons'];
         if($commons){
            $commons = explode('###|||###', $commons);
            esc_html__(anc_6310_multi_language_get('Number-Prefix', $commons[0]), 'animated-number-counters');
            esc_html__(anc_6310_multi_language_get('Number-Suffix', $commons[1]), 'animated-number-counters');
         }  
      }
   }
   
   function anc_6310_multi_language_get($field, $value, $id = ''){
      if (function_exists('icl_t')) {
         $id = $id ? $id . '. ' : '';
         $uId = "{$id}{$field}";
         icl_register_string('animated-number-counters', $uId, $value);
         $value = icl_t('animated-number-counters', $uId, $value);
      }	
      return anc_6310_special_character_remove(esc_attr(esc_html($value)));
   }
