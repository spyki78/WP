...
<table class="anc-6310-table">
    ...
    foreach ($data as $value) {
        ...
        echo '<td>' . esc_attr(ucfirst($style_name[0])) . " " . esc_attr($style_name[1]) . '</td>';
        echo '<td><span>Shortcode <input type="text" style="width: 250px;" onclick="this.setSelectionRange(0, this.value.length)" value="[anc_6310_number_counter id=&quot;' . esc_attr($id) . '&quot;]"></span>';
        echo '<td>
                                    <a href="' . admin_url("admin.php?page=anc-6310-counter-".esc_attr($temp)."&styleid=".esc_attr($id)) . '"  title="Edit"  class="anc-6310-btn-success anc-6310-margin-right-10" style="float:left; margin-right: 5px; margin-left: 5px; height: 17px"><i class="fas fa-edit" aria-hidden="true"></i></a>
                                    <form method="post">
