...
function anc_6310_shortcode_extract_function($ids) {
    ...
    if (file_exists(number_counter_with_skills_and_slider_plugin_url . "public/output-{$styledata['style_name']}.php")) {
        ...
        if (strpos(($styledata['style_name']), 'style') !== false) {
            wp_enqueue_style("anc-6310-google-font-" . esc_attr($ids), "https://fonts.googleapis.com/css?family=" . esc_attr(urlencode($allStyle['17'])) . "|" . esc_attr(urlencode($allStyle['24'])));
        } else if (strpos(($styledata['style_name']), 'square') !== false) {
            wp_enqueue_style("anc-6310-google-font-" . esc_attr($ids), "https://fonts.googleapis.com/css?family=" . esc_attr(urlencode($allStyle['12'])) . "|" . esc_attr(urlencode($allStyle['19'])));
        }
        include number_counter_with_skills_and_slider_plugin_url . "public/output-" . esc_attr($styledata['style_name']) . ".php";        
    }
}
